...
def train_KmeansModel_with_2features(df):
    #train kmeans clustering model on data (no train/test split for unsupervised model like kmeans)
    #inputs all data and returns dataframe with a new column listing what cluster it belongs to
...
def train_KmeansModel_with_3features(df):
    #similar as model above but using 3 features instead of just 2 and also produces silhouette score for different k values
    #inputs all data and returns the cluster and silhouetter scores
...
def elbowmethod_to_checkclusters(df):
    #loop through different values for k and determine best number of clusters for data
    #inputs data and returns metrics for silhouette and sum of square distances
...
def silhouttemethod_to_checkclusters(df,wss):
    #get silhouette score for each cluster number
